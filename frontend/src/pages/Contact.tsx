      ...
      <section id="contact-page-2" className="w-full bg-black py-8 border-t border-slate-900/60 min-h-[calc(100vh-6rem)] flex flex-col justify-center items-center">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-10 items-start w-full">
          ...
          <div className="lg:col-span-5 space-y-4">
            {/* Ready Card */}
            <div className="border border-white/20 bg-black p-6 rounded-2xl">
              <h2 className="scroll-reveal-target font-serif text-[22px] md:text-[28px] font-medium leading-tight text-white mb-3">
                Ready to transform your business?
              </h2>
              <p className="scroll-reveal-target text-white text-[14px] leading-relaxed mb-6">
            ...
            <div className="space-y-3">
              {/* Email Card */}
              <div className="border border-white/20 bg-black p-4 rounded-2xl flex items-center gap-4">
                <div className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center bg-black shrink-0 text-white">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                <div className="flex flex-col">
                  <span className="text-[12px] text-white font-medium mb-0.5">Email Us</span>
                  <a href="mailto:contact@galletrix.com" className="text-white text-[14px] font-semibold hover:underline">
              ...
              <div className="border border-white/20 bg-black p-4 rounded-2xl flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <span className="text-[12px] text-white font-medium mb-0.5">Call Us</span>
                  <a href="tel:+917305870929" className="text-white text-[14px] font-semibold hover:underline">
                    +91 73058 70929
                  </a>
                </div>
              </div>
              ...
              <div className="border border-white/20 bg-black p-4 rounded-2xl flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <span className="text-[12px] text-white font-medium mb-0.5">Visit us</span>
          ...
          <div className="lg:col-span-7 space-y-5">
            <h2 className="scroll-reveal-target font-serif text-[24px] md:text-[32px] font-medium text-white">
              Send US a Message
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-[13px] font-medium text-white/80">Full Name *</label>
                  <input 
                    ...
                    className="w-full bg-black border border-white/20 rounded-xl px-4 py-3 text-[14px] text-white placeholder-slate-600 focus:outline-none focus:border-[#0091ff] focus:ring-1 focus:ring-[#0091ff] transition-all" 
                    required 
                    disabled={status === 'submitting'}
                  />
                </div>
                <div className="space-y-1">
                  <label className="block text-[13px] font-medium text-white/80">Company</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="block text-[13px] font-medium text-white/80">Email *</label>
                  <input 
                    type="email" 
                ...
                <div className="space-y-1">
                  <label className="block text-[13px] font-medium text-white/80">Phone</label>
                  <div className="relative flex items-center w-full" ref={dropdownRef}>
                    ...
                    <div className="absolute left-0 top-0 bottom-0 flex items-center z-10">
                      <button
                        ...
                        className="flex items-center gap-1 px-3 h-full text-white hover:text-white rounded-l-xl focus:outline-none select-none border-r border-white/20 bg-[#05070a]/50"
                        disabled={status === 'submitting'}
                      >
                        <img loading="lazy" src={`https://flagcdn.com/w20/${selectedCountry.isoCode}.png`} alt={selectedCountry.name} className="w-4 object-contain rounded-sm" />
                        <span className="text-[13px] font-semibold">{selectedCountry.code}</span>
                    ...
                    <input 
                      ...
                      className="w-full bg-black border border-white/20 rounded-xl pl-[100px] pr-4 py-3 text-[14px] text-white placeholder-slate-600 focus:outline-none focus:border-[#0091ff] focus:ring-1 focus:ring-[#0091ff] transition-all" 
                      disabled={status === 'submitting'}
                    />

                    {/* Dropdown Menu */}
                    {isDropdownOpen && (
                      <div className="absolute top-full left-0 mt-1 w-72 max-h-48 overflow-y-auto bg-black border border-white/20 rounded-xl shadow-2xl z-50 py-1.5">
                        {countries.map((c) => (
                          <button
                            key={c.code + c.name}
                            type="button"
                            ...
                            className="w-full flex items-center gap-3 px-4 py-2 hover:bg-slate-900/80 transition-colors text-left"
                          >
                            <img loading="lazy" src={`https://flagcdn.com/w20/${c.isoCode}.png`} alt={c.name} className="w-5 object-contain rounded-sm" />
                            <div className="flex flex-col">
                              <span className="text-[13px] font-semibold text-white">{c.name}</span>
                              <span className="text-[11px] text-white font-medium">{c.code}</span>
                            </div>
                          </button>
                        ))}
                      </div>
              ...
              <div className="space-y-1">
                <label className="block text-[13px] font-medium text-white/80">Service of Interest</label>
                <div className="relative">
                  <select 
                    value={serviceOfInterest}
                    onChange={(e) => setServiceOfInterest(e.target.value)}
                    className="w-full bg-black border border-white/20 rounded-xl px-4 py-3 text-[14px] text-white focus:outline-none focus:border-[#0091ff] focus:ring-1 focus:ring-[#0091ff] transition-all appearance-none cursor-pointer"
                    disabled={status === 'submitting'}
                  >
              ...
              <div className="space-y-1">
                <label className="block text-[13px] font-medium text-white/80">Message*</label>
                <textarea 
                  rows={3} 
                  placeholder="Tell us about your projects, goals and challenges............." 
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  className="w-full bg-black border border-white/20 rounded-xl px-4 py-3 text-[14px] text-white placeholder-slate-600 focus:outline-none focus:border-[#0091ff] focus:ring-1 focus:ring-[#0091ff] transition-all resize-none" 
                  required 
                  disabled={status === 'submitting'}
                />
              </div>
              ...
              {status === 'success' && (
                <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 p-3 rounded-xl text-[13px] text-center font-medium animate-fade-in">
                  Thank you! Your message has been sent successfully. We will get back to you soon.
                </div>
              )}

              {status === 'error' && (
                <div className="bg-rose-500/10 border border-rose-500/30 text-rose-400 p-3 rounded-xl text-[13px] text-center font-medium animate-fade-in">
                  {errorMessage}
                </div>
              )}
              ...
              <button 
                type="submit" 
                disabled={status === 'submitting'}
                className="w-full bg-white hover:bg-slate-100 text-slate-900 py-3 rounded-xl text-[15px] font-semibold flex items-center justify-center gap-2 transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] cursor-pointer shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
